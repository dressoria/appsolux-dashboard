...
export function SriCenterBadge({
  label,
  variant = "neutral",
}: {
  label: string;
...
export function SriCenterMetricCard({
  icon: Icon,
  label,
  value,
  helper,
...
export function SriCenterActionCard({
  href,
  title,
  description,
  icon: Icon,
  ...
  icon: LucideIcon;
  status: string;
  statusVariant?: keyof typeof badgeClasses;
}) {
  return (
    <Link
      href={href}
    ...
    >
      ...
      <div className="mt-4 space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <SriCenterBadge label={status} variant={statusVariant} />
        </div>
...
export function SriCenterChecklistItem({
  label,
  value,
  href,
  actionLabel,

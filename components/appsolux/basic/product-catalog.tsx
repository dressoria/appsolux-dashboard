...
function ProductDrawer({ product, products, categories, onCategory, onClose }: { product?: CatalogProduct; products: CatalogProduct[]; categories: Category[]; onCategory: (category: Category) => void; onClose: () => void }) {
  ...
  async function createCategory(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const response = await fetch("/api/basic/product-categories", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: categoryName }) }); const result = await response.json() as { category?: Category; message?: string }; if (!response.ok || !result.category) return setError(result.message ?? "No se pudo crear."); onCategory(result.category); setCategoryId(result.category.id); setCategoryName(""); setCategoryDialogOpen(false); }
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { name: String(form.get("name") ?? ""), type, primaryCode: String(form.get("primaryCode") ?? ""), auxiliaryCode: String(form.get("auxiliaryCode") ?? ""), barcode: String(form.get("barcode") ?? ""), description: String(form.get("description") ?? ""), cost: number("cost"), price: number("price") ?? 0, price2: number("price2"), price3: number("price3"), taxRate: number("taxRate") ?? 0, categoryId, unit: String(form.get("unit") ?? "UNIT"), trackInventory, stock: number("stock") ?? 0, minStock: number("minStock"), expiresAt: String(form.get("expiresAt") ?? ""), isActive: form.get("isActive") === "on", iceEnabled, iceCode: String(form.get("iceCode") ?? ""), iceRate: number("iceRate"), comboItems: components };
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950/35" role="dialog" aria-modal="true"><div className="absolute inset-0" onClick={onClose}/><aside className="absolute inset-y-0 right-0 h-[100dvh] w-full overflow-y-auto overflow-x-hidden bg-white shadow-2xl sm:inset-y-4 sm:h-[calc(100dvh-2rem)] sm:max-w-3xl sm:rounded-l-3xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white px-5 py-4"><div><p className="text-lg font-bold text-slate-950">{product ? "Editar producto" : "Nuevo producto"}</p><p className="text-sm text-slate-500">Información comercial, tributaria e inventario.</p></div><Button variant="ghost" size="icon" onClick={onClose}><X className="h-5 w-5"/></Button></div>
    ...
    <label className="flex items-center gap-2 text-sm"><input name="isActive" type="checkbox" defaultChecked={product?.isActive ?? true}/>Activo y disponible para ventas</label>{error && <p className="text-sm text-red-600">{error}</p>}<div className="sticky bottom-0 flex justify-end gap-2 border-t bg-white py-4"><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button disabled={saving} className="bg-facturom-primary hover:bg-facturom-primary/90">{saving ? "Guardando…" : "Guardar producto"}</Button></div></form></aside>{categoryDialogOpen ? <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 p-4"><form onSubmit={createCategory} className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl"><h3 className="text-lg font-bold">Nueva categoría</h3><div className="mt-4 space-y-1.5"><Label htmlFor="category-name">Nombre</Label><Input id="category-name" value={categoryName} onChange={(event) => setCategoryName(event.target.value)} required autoFocus/></div><div className="mt-5 flex justify-end gap-2"><Button type="button" variant="outline" onClick={() => setCategoryDialogOpen(false)}>Cancelar</Button><Button className="bg-facturom-primary hover:bg-facturom-primary-soft">Guardar</Button></div></form></div> : null}</div>;
}

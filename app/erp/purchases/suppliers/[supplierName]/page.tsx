...
import { getSupplierHistory } from "@/lib/api/erpnext/party-history";
...
export default async function ErpSupplierHistoryPage({
  params,
}: {
      ...
      <DashboardShell>
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Inicia sesion para ver historial de proveedor.
          </p>
        </div>
      </DashboardShell>
    );
      ...
      <DashboardShell>
        <Card>
          <CardContent className="p-6 text-sm text-muted-foreground">
            <p>El ERP dedicado es necesario para ver historial de proveedores.</p>
          </CardContent>
        </Card>
      </DashboardShell>
    );
  ...
  const history = await getSupplierHistory(decodedSupplierName);

  if (!history.supplier && history.invoices.length === 0) {
    notFound();
  }
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h1 className="text-3xl font-semibold tracking-tight">
              {history.supplier?.supplier_name ?? decodedSupplierName}
            </h1>
            <p className="mt-2 text-muted-foreground">
              Historial ERPNext de facturas, pagos, ordenes de compra e ingresos
              de mercaderia del proveedor.
            </p>
          </div>
        ...
        <div className="grid gap-3 md:grid-cols-4">
          <Card>
            ...
            <CardContent className="text-2xl font-semibold">
              {formatMoney(history.totalPurchased)}
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
          ...
          <CardContent>
            {history.invoices.length === 0 ? (
              <div className="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">
                Este proveedor aun no tiene facturas de compra.
              </div>
            ) : (
                      ...
                      <tr key={invoice.name}>
                        ...
                        <td className="py-2">
                          <div className="flex flex-wrap gap-1.5">
                            <DocumentActions doctype="Purchase Invoice" name={invoice.name} size="xs" />
        ...
        <section className="grid gap-6 xl:grid-cols-3">
          <Card>
            <CardHeader>
              <CardTitle>Pagos a proveedor</CardTitle>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {history.purchaseOrders.length === 0 ? (
                ...
                history.purchaseOrders.slice(0, 8).map((order) => (
          ...
          <Card>
            <CardHeader>
              <CardTitle>Ingresos de mercaderia</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2 text-sm">
              {history.purchaseReceipts.length === 0 ? (
                <p className="text-muted-foreground">No hay ingresos registrados.</p>
              ) : (
                history.purchaseReceipts.slice(0, 8).map((receipt) => (
                  <div key={receipt.name} className="rounded-lg border p-3">
                    <p className="font-medium">{receipt.name}</p>
                    <p className="text-muted-foreground">
                      {receipt.posting_date ?? "-"} · {receipt.status ?? "-"}
                    </p>
                    <p>{formatMoney(receipt.grand_total)}</p>
                  </div>
                ))
              )}
            </CardContent>

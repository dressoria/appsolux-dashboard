...
export default async function BasicHomePage() {
  ...
  if (!user) {
    return (
      <BasicModuleShell
        title="Centro de operacion basica"
        description="Inicia sesion para vender, cobrar y controlar stock."
        activeHref={routes.basic}
      >
  ...
  return (
    <BasicModuleShell
      title="Centro de operacion basica"
      description="Tu punto de venta diario: vende, cobra, revisa fiados y cuida tu inventario desde Core DB."
      activeHref={routes.basic}
    >
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <Card>
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ["Nueva venta", routes.basicPos],
              ["Agregar producto", routes.basicProducts],
              ["Agregar cliente", routes.basicCustomers],
              ["Ver ventas", routes.basicSales],
              ["Caja diaria", routes.basicCash],
              ["Movimientos de stock", routes.basicStock],
            ].map(([label, href]) => (

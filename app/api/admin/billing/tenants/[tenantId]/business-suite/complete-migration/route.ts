...
import {
  completeBusinessSuiteMigration,
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const allowWarnings = body.allowWarnings === true;
    ...
    const result = await completeBusinessSuiteMigration({
      actorUserId: user.id,
      tenantId,
      businessSuiteMode,
      allowWarnings,
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "No se pudo completar la migracion operativa.",
      },
      { status: 400 }
    );
  }
